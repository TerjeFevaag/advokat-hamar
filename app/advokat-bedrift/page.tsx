...
import pageData from '@/content/bedrift.json'
...
export default function BedriftPage() {
  // Flatten all services with a global index for watermark numbers
  let globalIndex = 0
        ...
        <section className="py-20 px-8 md:px-20 bg-white border-b border-gray-100">
          <div className="max-w-6xl mx-auto">
            <div className="flex items-center gap-3 text-gold text-xs font-bold uppercase tracking-[0.2em] mb-5">
              <span className="w-10 h-px bg-gold block" />
              {pageData.selskapsrettIntro.tag}
            </div>
            <h2 className="font-serif text-[clamp(1.75rem,3vw,2.75rem)] font-bold text-navy leading-tight mb-6">
              {pageData.selskapsrettIntro.title}
            </h2>
            <div className="space-y-4">
              {pageData.selskapsrettIntro.paragraphs.map((p, i) => (
          ...
          <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
            <div>
              <h2 className="font-serif text-3xl font-bold text-white mb-2">
                Trenger bedriften din juridisk bistand?
              </h2>
              <p className="text-white/55 leading-relaxed">
                Vi tilbyr gratis første samtale. Kontakt oss i dag for en uforpliktende prat.
              </p>
            </div>
            <a
              href="/kontakt"
